...
MAINLOOP: while (index($line, "$boundary--") == -1) {
	...
	&$cbfunc($got, $ENV{'CONTENT_LENGTH'},
		undef, $in{'id'});
	if ($line =~ /(\S+):\s*form-data(.*)$/) {
		# We found form data definition, let's check it
		$rest = $2;
		}
	else {
		next;
	...
	if (defined($file)) {
		...
		if ($file =~ /\//) {
			...
			if ($dir) {
				...
				if (!$in{'overwrite_existing'}) {
					if ($dirs[0] &&
					    -e "$cwd/$dirs[0]") {
						# As only one directory upload
						# at a time allowed check if
		...
		if (!open(OUTFILE, ">$full")) {
			...
			# Skip "content-type" as we work in
			# binmode anyway and skip empty line
			<STDIN>;
			<STDIN>;

			# Read all lines until next boundary
			# or form data end
			while (1) {
				$line = <STDIN>;
				if (!defined($line)) {
		...
		# Just skip everything until next boundary
		# or form end
